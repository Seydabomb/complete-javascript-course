...
// // Prefixed ++ operator
// let a = 10;
// console.log(++a); // outputs 11 if ++a, outputs 10 if a++
...
//   .flatMap(acc => acc.movements)
//   .reduce(
//     // the arrow function doesn't have an implicit return because it is using {} so we have to manually return it.
//     (sums, cur) => {
...
//   ); // sums is the accumulator and it is the {deposits: 0, withdrawals: 0}
...
//     .join(' '); // word.slice(1) the rest of the string is input in starting at position number 1, since position zero was manipulated
//   // exceptions.includes(word). If the array has the word then output the word, if it doesn't then manipulate
...
// dogs.forEach(dog => (dog.recFood = dog.weight ** 0.75 * 28)); // creates new property named recFood: the calculated number.
// console.log(dogs);
...
// console.log(`${dogs.some(dog => dog.curFood === dog.recFood)}`); // 'any' dog so that tells us we use the some() method
